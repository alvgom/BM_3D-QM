function [X_um0, X_voxel0, X_um_rw, X_voxel_rw, X_um0_all, X_voxel0_all, X_um_rw_all, X_voxel_rw_all] = GetSpotPositions_voxel(this,vSpot,mask)
...
if nargin < 3
    mask = ones(round(imsizex),round(imsizey),round(imsizez1));
end
...
count_pp = 0;
pp_posmask = [];
for pp = 1:length(X_voxel0_all)
    if mask(round(X_voxel0_all(pp,1)),round(X_voxel0_all(pp,2)),round(X_voxel0_all(pp,3)))
        count_pp = count_pp + 1;
